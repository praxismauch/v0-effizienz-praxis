...
CREATE POLICY "Users can insert appraisals in their practice"
  ON employee_appraisals FOR INSERT
  WITH CHECK (
    practice_id IN (
      SELECT practice_id FROM users WHERE id = auth.uid()
    )
...
CREATE POLICY "Users can update appraisals in their practice"
  ON employee_appraisals FOR UPDATE
  USING (
    practice_id IN (
      SELECT practice_id FROM users WHERE id = auth.uid()
    )
...
CREATE POLICY "Users can delete appraisals in their practice"
  ON employee_appraisals FOR DELETE
  USING (
    practice_id IN (
      SELECT practice_id FROM users WHERE id = auth.uid()
    )
...
-- Grant permissions
GRANT ALL ON employee_appraisals TO authenticated;
GRANT ALL ON employee_appraisals TO service_role;
