...
import { PermissionsLoading, PermissionsError, PermissionsEmpty } from "./user-rights/permission-states"
...
export function UserRightsManager() {
  ...
  const savePermissions = async () => {
    try {
      ...
      if (failedCount > 0) throw new Error(`${failedCount} Berechtigungen konnten nicht gespeichert werden`)
      setModifiedPermissions(new Set())
      toast({ title: "Erfolgreich gespeichert", description: `${permissionsToSave.length} Berechtigungen wurden aktualisiert.` })
    } catch (err) {
      console.error("Error saving permissions:", err)
      toast({ title: "Fehler beim Speichern", description: err instanceof Error ? err.message : "Berechtigungen konnten nicht gespeichert werden", variant: "destructive" })
    } finally {
      setSaving(false)
    }
  }
  ...
  const initializePermissions = async () => {
    try {
      ...
      setError(null)
      const response = await fetch("/api/role-permissions/initialize", { method: "POST", headers: { "Content-Type": "application/json" } })
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}))
        throw new Error(errorData.error || "Failed to initialize permissions")
      }
      const data = await response.json()
      toast({ title: "Berechtigungen initialisiert", description: `${data.count || 0} Standard-Berechtigungen wurden erstellt.` })
      ...
      toast({ title: "Fehler", description: "Berechtigungen konnten nicht initialisiert werden", variant: "destructive" })
    } finally {
  ...
  if (error) return <PermissionsError error={error} onRetry={loadPermissions} onInitialize={initializePermissions} initializing={initializing} />
  if (permissions.length === 0) return <PermissionsEmpty onInitialize={initializePermissions} initializing={initializing} />
  ...
  const permissionsByKey = Object.entries(groupedPermissions).map(([category, perms]) => {
    ...
    perms.forEach((perm) => {
      if (!byKey[perm.permission_key]) byKey[perm.permission_key] = {}
    ...
    <div className="w-full space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={savePermissions} disabled={modifiedPermissions.size === 0 || saving}>
            ...
            {saving ? "Speichert..." : `Speichern${modifiedPermissions.size > 0 ? ` (${modifiedPermissions.size})` : ""}`}
          </Button>
        </div>
      </div>
      ...
      <Tabs value={activeCategory} onValueChange={setActiveCategory} className="w-full">
        <TabsList className="w-full flex-wrap h-auto gap-1 p-1">
          <TabsTrigger value="all" className="text-xs sm:text-sm">Alle</TabsTrigger>
                          ...
                          <React.Fragment key={key}>
                            {actions.map((action, actionIndex) => {
                              ...
                              return (
                                <tr
                                  key={`${key}-${action.key}`}
                                  className={`border-b hover:bg-muted/30 transition-colors ${actionIndex === 0 ? "border-t-2 border-t-muted" : ""}`}
                                >
                                  {actionIndex === 0 && (
                                    <td rowSpan={actions.length} className="p-3 font-medium capitalize border-r align-middle bg-muted/20 whitespace-nowrap">
                                  ...
                                  {roles.map((role) => {
                                    ...
                                    if (!perm) return <td key={`${key}-${role}`} className="p-2 text-center">-</td>
                                    const fieldKey = action.key as "can_view" | "can_create" | "can_edit" | "can_delete"
                                    return (
                                      <td key={`${key}-${role}-${action.key}`} className="p-2 text-center">
                                        <div className="flex justify-center">
                                          <Checkbox
                                            checked={perm[fieldKey]}
                                            onCheckedChange={(checked) => handlePermissionChange(perm.id, fieldKey, checked as boolean)}
