  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log("[v0] ROI Edit - Update successful:", updatedAnalysis)
        setAnalysis(updatedAnalysis)
        setIsEditing(false)
        onUpdated()
      } else {
        const error = await response.json()
        console.error("[v0] ROI Edit - Update failed:", error)
