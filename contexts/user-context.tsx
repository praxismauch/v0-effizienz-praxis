...
export function UserProvider({
  ...
  useEffect(() => {
    ...
    if (currentUser) {
      if (IS_DEBUG) {
        console.debug("[v0] UserProvider: User already loaded:", currentUser.id)
      }
      setIsLoading(false)
      return
    }
    ...
    const loadUserFromAuth = async () => {
      if (IS_DEBUG) {
        console.debug("[v0] UserProvider: Attempting to load user from Supabase auth")
      }
      try {
        ...
        if (!supabase) {
          if (IS_DEBUG) {
            console.debug("[v0] UserProvider: Supabase client not available")
          }
          setIsLoading(false)
          return
        }
        ...
        if (authError || !authUser) {
          if (IS_DEBUG) {
            console.debug("[v0] UserProvider: No auth user found:", authError?.message)
          }
          setIsLoading(false)
          return
        }
        ...
        if (IS_DEBUG) {
          console.debug("[v0] UserProvider: Auth user found:", authUser.id)
        }
        ...
        if (profileError || !profile) {
          if (IS_DEBUG) {
            console.debug("[v0] UserProvider: No profile found:", profileError?.message)
          }
          setIsLoading(false)
          return
        }
        ...
        if (IS_DEBUG) {
          console.debug("[v0] UserProvider: User profile loaded:", user.id, "practiceId:", user.practice_id)
        }
        ...
        try {
          localStorage.setItem("effizienz_current_user", JSON.stringify(user))
        } catch (e) {
          if (IS_DEBUG) {
            console.debug("[v0] UserProvider: Error storing user:", e)
          }
        }
      } catch (err) {
        console.error("[v0] UserProvider: Error loading user:", err)
      } finally {
        setIsLoading(false)
        setIsAuthInitialized(true)
      }
