...
DROP TRIGGER IF EXISTS trigger_update_cirs_incidents_updated_at ON cirs_incidents;
...
DROP POLICY IF EXISTS "Users can view incidents in their practice" ON cirs_incidents;
DROP POLICY IF EXISTS "Users can create incidents in their practice" ON cirs_incidents;
DROP POLICY IF EXISTS "Users can update incidents in their practice" ON cirs_incidents;
DROP POLICY IF EXISTS "Users can view comments on incidents they can see" ON cirs_incident_comments;
DROP POLICY IF EXISTS "Users can create comments on incidents they can see" ON cirs_incident_comments;

-- RLS Policies for cirs_incidents (using authenticated role)
...
CREATE POLICY "Users can delete incidents in their practice"
  ON cirs_incidents FOR DELETE
  TO authenticated
  USING (true);
