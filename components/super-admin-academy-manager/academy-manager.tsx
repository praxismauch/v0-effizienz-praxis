              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {a.badges.map((badge) => (
                  <Card key={badge.id} className="overflow-hidden"><CardContent className="pt-6"><div className="flex flex-col items-center text-center">
                    ...
                    {badge.criteria_type && <div className="mt-2"><Badge variant="outline" className="text-xs text-primary cursor-pointer hover:bg-primary/10 transition-colors" onClick={() => a.setActiveTab("triggers")}><Zap className="h-3 w-3 mr-1" />{CRITERIA_TYPES.find((c) => c.value === badge.criteria_type)?.label || badge.criteria_type}{badge.criteria_value ? `: ${badge.criteria_value}` : ""}</Badge></div>}
