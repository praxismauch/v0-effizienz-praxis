-- Migration to restore global KPI categories and fix schema issues
-- Run this script to ensure all global KPI categories and templates are properly set up
...
CREATE TABLE IF NOT EXISTS global_parameter_templates (
  ...
  name TEXT NOT NULL,
  description TEXT,
  type TEXT NOT NULL CHECK (type IN ('number', 'percentage', 'currency', 'boolean', 'text', 'select', 'date', 'time', 'calculated')),
...
ALTER TABLE global_parameter_templates ADD COLUMN IF NOT EXISTS group_ids TEXT[];
ALTER TABLE global_parameter_templates ADD COLUMN IF NOT EXISTS usage_count INTEGER DEFAULT 0;

-- 3. Fix user_sidebar_preferences table if it exists
ALTER TABLE user_sidebar_preferences ADD COLUMN IF NOT EXISTS favorites JSONB DEFAULT '[]'::jsonb;
ALTER TABLE user_sidebar_preferences ADD COLUMN IF NOT EXISTS collapsed_sections TEXT[] DEFAULT '{}';
ALTER TABLE user_sidebar_preferences ADD COLUMN IF NOT EXISTS single_group_mode BOOLEAN DEFAULT true;

-- 4. Fix user_favorites table if it exists
...
CREATE INDEX IF NOT EXISTS idx_global_parameter_groups_practice_id ON global_parameter_groups(practice_id);
CREATE INDEX IF NOT EXISTS idx_global_parameter_groups_display_order ON global_parameter_groups(display_order);
...
-- 8. Re-seed default global KPI categories (upsert)
...
-- 9. Re-seed default global KPI templates (upsert)
...
SELECT 'Global KPI Categories:' as info;
SELECT id, name, color, display_order FROM global_parameter_groups WHERE practice_id IS NULL ORDER BY display_order;

SELECT 'Global KPI Templates:' as info;
SELECT id, name, category, type FROM global_parameter_templates WHERE is_template = true ORDER BY category, name;
